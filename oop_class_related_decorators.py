...
 Description 
You need to create abstract class Vehicle. Classes Car, Motorcycle, Truck, Bus are inherited from Vehicle and already implemented. 
...
vehicle_type - returns str - type of the vehicle in the following pattern brand_name + name of the class. For example: Toyota Car, Suzuki
Motorcycle;
is_motorcycle return boolean value depends on the amount of wheels (2 wheels -> motorcycle, so method should return True);
purchase_price - returns the price of the vehicle: (base_price - 0.1 * mileage). If the result price is less than 100_000, method should
return 100_000. 
...
class Vehicle(ABC):
    ...
    def purchase_price(self) -> float:
        price = self._base_price - 0.1 * self._mileage
