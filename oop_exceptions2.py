...
 Description 
Write a function divide which accepts a string that contains two integers, separated by spaces (integers can be separated by more than one space). You
have to perform the division operation (a / b) and return the result (float) or an error message. 
...
def divide(str_with_ints: str) -> Union[float, str]:
    ...
    if len(nums) != 2:
        return "Error code: expected two numbers"
